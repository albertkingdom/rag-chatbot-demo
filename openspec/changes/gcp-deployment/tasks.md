...
- [x] 9.2 建立 `.github/workflows/deploy.yml`：workflow 觸發條件設為 `on: push: branches: ['release/**']`（master push 不觸發），執行 WIF 認證 → docker build（tag 為 git SHA）→ push 到 Artifact Registry → `terraform init` → **scoped** `terraform apply -auto-approve -target=google_cloud_run_v2_service.web -target=google_cloud_run_v2_job.sync -var="image_tag=$GITHUB_SHA"`（只 apply app 層，不動基礎設施）— 滿足「GitHub Actions deploys to Cloud Run on push to release branch」「CI deployer service account holds least-privilege roles only」「Docker image tagged with git SHA」需求；驗證：建立 `release/**` branch 並 push，workflow 全綠且 apply 輸出不含任何基礎設施 add/change/destroy；push master 確認不觸發
...
- [ ] 9.4 **bootstrap：啟用 CI 必要 API**（手動，因牽涉 deployer SA 自身權限、屬雞生蛋不納 CI）：`gcloud services enable iam.googleapis.com iamcredentials.googleapis.com`，確認 `gcloud services list --enabled` 含此二者 — 滿足「Required GCP APIs are enabled」需求
- [ ] 9.5 **bootstrap：授予 deployer SA 最小權限**（手動）：除既有 `run.admin` + `artifactregistry.writer`，加 runtime SA 上的 `roles/iam.serviceAccountUser`（actAs）、專案層 `roles/secretmanager.viewer`（只讀 secret metadata）、tfstate bucket 的 `roles/storage.objectAdmin`、共享資料 bucket 的 `roles/storage.legacyBucketReader`；**不給** owner/editor/任何 IAM 或 secret 寫入角色 — 滿足「CI deployer service account holds least-privilege roles only」需求；驗證：CI 的 scoped apply 能更新 Cloud Run image，且 deployer 無法建立/修改 SA、IAM、secret、WIF
